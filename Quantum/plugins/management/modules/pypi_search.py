...
from Quantum.plugins.management.helper.media_helper import post_to_telegraph
from Quantum.plugins.management.helper.sqlite_helper import Cache
...
async def getDataPypi(msg, kueri, CurrentPage, user):
    if not PYPI_DICT.get(msg.id):
        ...
        if not pypijson.get("result"):
            await msg.edit_text("Sorry could not find any matching results!", del_in=6)
            return None, 0, None
...
async def pypi_s(_, ctx: Message):
    ...
    pesan = await ctx.reply_text("⏳ Please wait, getting data from pypi..", quote=True)
    CurrentPage = 1
    ...
    await pesan.edit_text(pypires, reply_markup=keyboard)
...
async def pypipage_callback(_, callback_query: CallbackQuery):
    ...
    await callback_query.message.edit_text(pypires, reply_markup=keyboard)
...
async def pypi_getdata(_, callback_query: CallbackQuery):
    ...
    try:
        ...
        try:
            ...
            await callback_query.message.edit_text(f"Result is too long:\n{url}")
    except Exception as err:
        await callback_query.message.edit_text(f"ERROR: {err}", reply_markup=keyboard)